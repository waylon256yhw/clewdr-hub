...
| Phase | 内容 | 风险 | 依赖 |
|-------|------|------|------|
| **0** | Fork + 清理（删 Web/OAI，单 endpoint 跑通） | 低 | 无 |
| **1** | SQLite 集成 + 数据模型 + migrations + admin 初始化 | 低 | Phase 0 |
...
- 引入 `sqlx` + SQLite WAL 模式
- 建表迁移（§4 的完整 schema）
- Seed data（default policy + model_pricing）
- Admin 用户初始化（`ADMIN_PASSWORD` 环境变量 → argon2 hash）
- 启动时自动 migrate
