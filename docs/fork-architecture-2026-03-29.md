...
> 更新：2026-03-30（rev.4，Phase 0-1 实现完成，补充实现决策和 schema 修正）
...
|------|-------------|---------------|--------------------|--------------------|
| Opus 4 | $15 | $75 | 15,000 | 75,000 |
| Sonnet 4 | $3 | $15 | 3,000 | 15,000 |
...
| **2** | 多用户认证（API key 链路 + admin session cookie） | 中 | Phase 1 | |
| **3** | Scheduler 重构（round-robin try_acquire + cooldown + lease） | 中 | Phase 2 | |
| **4** | 计费系统（usage 解析 → cost → rollup + 日志清理） | 中 | Phase 3 | |
...
- **AppState 统一**：引入 `AppState { db: SqlitePool, cookie_actor: CookieActorHandle, provider: Arc<ClaudeCodeProvider> }`，
  使用 Axum `FromRef` 让 handler 提取子状态，替代当前 per-route `.with_state()` 模式
- API key 生成/验证（blake3 hash + lookup_key 前缀查询）
- 入站认证中间件：基于已有 `RequireFlexibleAuth`（[clewdr#130](https://github.com/Xerxes-2/clewdr/pull/130)）改造，
  将原来的 admin 密码比对替换为 API key 查表验证，保持 `x-api-key` / `Bearer` 双格式兼容
- Admin session：密码登录 → HMAC 签名 HTTP-only cookie + CSRF
- 此时请求链路变为：authenticate → load policy → (暂跳过配额检查) → 原有转发逻辑

#### Phase 3: Scheduler 重构

clewdr 已有 `CookieStatus` 完整的窗口管理和 cooldown 逻辑，此阶段在外层包装：
- **accounts 表迁移**：新增 `migrations/0002_accounts.sql`，创建 accounts 表 + `request_logs.account_id` 列
- `AccountPool`：round-robin 遍历 + `try_acquire` 原子选择（Semaphore per-account）
- `AccountLease`：RAII guard，Drop 时释放 slot
- per-user 并发控制（tokio Semaphore）
- per-user RPM 滑动窗口（内存 `HashMap<UserId, VecDeque<Instant>>`）
- 全部 busy → 等 2-5s 重试一轮 → 仍失败则 429

**不动**：`CookieStatus` 内部的 token 桶、窗口重置、cooldown 定时器——这些原封保留。

#### Phase 4: 计费系统

工作量比预期更小——不是从零建系统，而是在已有 token 解析管道上接金额层：

1. **`normalize_model()`**：将 `model` 字符串映射到 `pricing_key`（比 `classify_model()` 更精确）
2. **计费 hook**：在 `persist_usage_totals()` 末尾插入——拿到 `(input_tokens, output_tokens, model_raw)` 后：
   - `normalize_model()` → 查 `model_pricing` 表获取单价
   - 算 `cost_nanousd = input × input_price + output × output_price`
   - INSERT `request_logs`（含价格快照）
   - UPSERT `usage_rollups`（周/月滚动）
3. **配额检查**：在 gateway 请求链路中（Phase 2 留的 hook 点），查 rollup 判断 soft cap
4. **日志清理定时器**：启动时 + 每日删除 >7 天的 `request_logs`

已有 `classify_model()` 保持不变（账户级 token 桶继续用它），`normalize_model()` 仅服务于计费层。
