...
但指纹完整度不足。此阶段补齐所有 HTTP header 指纹 + 动态版本管理 + 前端版本配置。
...
Admin Settings 页面暴露 `cc_cli_version` 等配置，用户可直接修改，保存即生效无需重启。

版本号信息源：后台定时抓取 `https://api.github.com/repos/anthropics/claude-code/releases`
（或 npm registry `https://registry.npmjs.org/@anthropic-ai/claude-code`），提取最近 5 个版本号
展示在设置页面作为参考/下拉选项。轮转周期 6-12h，缓存到 settings 表（如 `cc_latest_versions` key）。

| 信息源 | 优先级 | 说明 |
|--------|--------|------|
| GitHub Releases API | 首选 | `/repos/anthropics/claude-code/releases?per_page=5`，取 `tag_name` |
| npm Registry | 备选 | `/@anthropic-ai/claude-code`，取 `dist-tags` + `versions` |
...
- 后台定时任务：抓取最新版本号，更新 `cc_latest_versions` setting
- Admin API：`GET /api/admin/settings` 返回含版本建议列表

**工作量**：后端约 150 行（header 组装 + 版本抓取），前端在 Phase 6 Settings 页面一并实现。
