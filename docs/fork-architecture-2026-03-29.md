...
> 更新：2026-03-31（rev.10，Phase 0-5 实现完成，Phase 5.5 计划）
...
| **5.5** | CookieActor DB 化（消除 .toml split-brain） | 高 | Phase 5 | |
| **6** | 前端重写（admin panel） | 中 | Phase 5.5 | |
...
> 目标：消除 .toml ↔ DB split-brain，统一账号管理为单一数据源

**问题**：Phase 5 的 `/api/admin/accounts` 写 DB，但运行时 CookieActor 从 `.toml` 加载、
往 `.toml` 回写。两套系统完全不通——Admin API 创建的账号不参与调度，删除的账号继续服务。

**设计决策**（Codex 协商）：

| 问题 | 决策 | 理由 |
|------|------|------|
| 哪个方案 | Option B：DB-backed Actor | 消除 split-brain，不换并发模型 |
| Usage 桶持久化 | 是 | 重启丢失 = 丢失 Anthropic 限额记忆，增加 429 风险 |
| 存哪个表 | 分表：`accounts`（admin）+ `account_runtime_state`（actor） | 不同所有权、不同写入频率 |
| 写入策略 | dirty flag + 定时 flush（~15s）+ 状态变更时 flush | 不同步写每次请求 |
| Moka cache | 保留，纯内存 | 路由优化，不持久化 |
| .toml cookie_array | 移除运行时权威 | fallback = split authority |
| CookieStatus struct | 保留为内部结构，只改 I/O | 最小侵入，调度逻辑不动 |
| accounts.status | 拆分 admin 意图 vs 运行时健康 | `disabled_at`（人控）+ `dispatch_status`（系统） |

**新建 `account_runtime_state` 表**：

```sql
CREATE TABLE account_runtime_state (
    account_id INTEGER PRIMARY KEY REFERENCES accounts(id) ON DELETE CASCADE,
    dispatch_status TEXT NOT NULL CHECK (
        dispatch_status IN ('active', 'cooldown', 'auth_error')
    ) DEFAULT 'active',
    reset_time INTEGER,                    -- cooldown until (epoch)

    -- 1M 支持标志（运行时探测）
    supports_claude_1m_sonnet INTEGER,     -- tri-state: NULL/0/1
    supports_claude_1m_opus INTEGER,
    count_tokens_allowed INTEGER,

    -- 5 个 Usage 桶 × 6 计数器 = 30 列
    ...
    weekly_sonnet_total_input INTEGER NOT NULL DEFAULT 0,
    weekly_sonnet_total_output INTEGER NOT NULL DEFAULT 0,

    weekly_opus_total_input INTEGER NOT NULL DEFAULT 0,
    weekly_opus_total_output INTEGER NOT NULL DEFAULT 0,
    ...
    -- 窗口追踪标志（tri-state: NULL=未知, 0=不追踪, 1=追踪）
...
**accounts 表 status 拆分**：
- 移除 `status` 列的 `cooldown` / `auth_error` 值（运行时状态不应存 admin 表）
- `accounts.disabled_at` 替代 `status = 'disabled'`（admin 意图）
- 运行时健康状态存 `account_runtime_state.dispatch_status`

**CookieActor 改造**：

1. **传入 SqlitePool**：`CookieActorHandle::start(pool)` → Actor `Arguments = SqlitePool`
2. **pre_start 从 DB 加载**：
   ```
   SELECT a.*, rs.* FROM accounts a
   LEFT JOIN account_runtime_state rs ON a.id = rs.account_id
   WHERE a.disabled_at IS NULL
   ```
   → 映射为 `CookieStatus`（保留内部结构）
3. **新增 ReloadFromDb 消息**：
   - 从 DB 重新加载账号列表
   - 按 `cookie` 字段匹配现有内存条目，**merge 保留 usage 桶**
   - 新增的账号初始化为空桶
   - 被删除的账号从内存移除
4. **save() 改为写 DB**：
   - 替换 9 处 `Self::save(state)` 调用
   - 使用 dirty flag，仅写变更的账号
   - 定时 flush（复用 CheckReset 定时器，每 300s）
   - 状态变更（cooldown/auth_error）立即 flush 该条目
5. **移除 .toml 持久化**：
   - 删除 `CLEWDR_CONFIG.rcu()` + `.save()` 调用链
   - 删除 `ClewdrConfig.cookie_array` / `wasted_cookie` 字段
   - Legacy `/api/cookie*` 端点改为操作 DB + 调用 ReloadFromDb
   - `.toml` 仅保留非 cookie 配置（password、端口等）

**Admin API 联动**：
- `/api/admin/accounts` CRUD 操作后调用 `CookieActorHandle::reload_from_db()`
- Actor 内部 merge 逻辑保证 CRUD 不丢失运行时 usage 桶

**不动的部分**：
- CookieStatus 内部字段和方法
- 调度逻辑（round-robin FIFO + moka cache）
- Usage 解析管道（`persist_usage_totals` → `add_and_bucket_usage`）
- 429 恢复机制（`Reason::TooManyRequest` → exhausted → reset）
- 窗口刷新逻辑（`refresh_usage_windows`）

**涉及文件**（~10 个）：

| 文件 | 改动 |
|------|------|
| `migrations/新文件` | account_runtime_state 表 + accounts 改造 |
| `src/services/cookie_actor.rs` | pre_start/save/新消息（核心） |
| `src/router.rs` | CookieActorHandle::start(pool) |
| `src/config/clewdr_config.rs` | 移除 cookie_array / wasted_cookie |
| `src/config/cookie.rs` | CookieStatus ↔ DB 映射方法 |
| `src/api/misc.rs` | legacy endpoint 改为 DB + reload |
| `src/api/admin/accounts.rs` | CRUD 后调 reload |
| `src/api/admin/overview.rs` | 展示 runtime state |
| `src/claude_code_state/mod.rs` | 可能微调（传递变化） |
| `src/db/mod.rs` | account runtime CRUD 查询 |

**预估**：~650 行改动，一个 session 可完成（保留 CookieStatus 内部结构是关键简化）。
