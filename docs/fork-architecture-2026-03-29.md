...
> 更新：2026-04-01（rev.14，Phase 6 admin panel 前端实现完成）
...
/users                          -- 用户管理（Tabs: 用户/策略 + Keys Drawer）
/settings                       -- 设置（CLI 版本下拉 + 代理 + 改密码）
/logs                           -- 请求日志（服务端分页 + 筛选 + 详情 Drawer）
```

### 文件结构（12 文件，feature-flat）

```
frontend/src/
  api.ts              -- apiFetch + 全部端点函数 + TS 类型 + query keys
  ...
  routes/
    ...
    Users.tsx         -- 用户/策略 Tabs + Keys Drawer（~500 行，最复杂页面）
...
1. 用户输入用户名密码 → `POST /auth/login` → 返回 `api_key`（24h 过期）+ `must_change_password`
2. 前端存 `api_key` 到 `localStorage`，后续请求注入 `Authorization: Bearer` 头
3. 首次登录（默认密码）→ 进入主页后弹出不可关闭的改密码 Modal，当前密码预填
4. 改密码 → `PUT /api/admin/me/password` → 清除 `must_change_password` + 撤销其他 web-session key
5. 退出 → `POST /auth/logout` 删除当前 key + 清 localStorage + 清 QueryCache
6. 401 全局处理 → `window.dispatchEvent('auth:logout')` → AuthProvider 响应
7. 页面刷新 → `GET /api/admin/overview` 验证 token + 检查 `must_change_password`

### 设计决策

| 决策 | 选择 | 理由 |
|------|------|------|
| 文件组织 | feature-flat（12 文件） | 单维护者，不预拆，超 300 行再考虑 |
