...
async fn load_bindings_for_keys(db: &SqlitePool, key_ids: &[i64]) -> Result<Vec<BindingRow>, sqlx::Error> {
    if key_ids.is_empty() {
        return Ok(Vec::new());
    }
    let placeholders: String = key_ids.iter().map(|_| "?").collect::<Vec<_>>().join(",");
    let sql = format!("SELECT api_key_id, account_id FROM api_key_account_bindings WHERE api_key_id IN ({placeholders})");
...
fn attach_bindings(keys: Vec<KeyRow>, bindings: Vec<BindingRow>) -> Vec<KeyResponse> {
    keys.into_iter()
        .map(|k| {
            let bound: Vec<i64> = bindings.iter().filter(|b| b.api_key_id == k.id).map(|b| b.account_id).collect();
            KeyResponse {
                id: k.id,
                user_id: k.user_id,
                username: k.username,
...
pub async fn create(
    ...
    for aid in &bound {
        sqlx::query("INSERT INTO api_key_account_bindings (api_key_id, account_id) VALUES (?1, ?2)")
            .bind(ak_id)
            .bind(aid)
            .execute(&db)
            .await?;
