...
- API 密钥页创建成功后改为始终展示完整 Key，并继续尝试自动复制；列表里的隐式点击复制收敛为显式「查看/复制」入口。
- API 密钥复制链路统一到新的前端 clipboard helper，优先使用 `navigator.clipboard`，失败时回退到兼容复制逻辑。
- API 密钥创建/查看 modal 针对桌面端收紧尺寸，保留移动端长按手动复制的可用性。
